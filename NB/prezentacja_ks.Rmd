...
```{r}
napisy_df <- data.frame(napis = c('aaa', '1224445', '', NA, 'abc123')
                        , stringsAsFactors = F)
napisy_df$`dopasowanie .+` <- str_extract(napisy_df$napis, '.+')
napisy_df$`dopasowanie .*` <- str_extract(napisy_df$napis, '.*')
napisy_df$`dopasowanie .?` <- str_extract(napisy_df$napis, '.?')
pander::pander(napisy_df)
```
...
napisy_df$`dopasowanie .{2,4}` <- str_extract_all(napisy_df$napis, '.{3,4}')
pander::pander(napisy_df)
```
...
```{r results='asis'}
napisy_df <- data.frame(napis = c('abc', '12345', 'a-1', NA, 'abc123', '\t')
                        , stringsAsFactors = F)
